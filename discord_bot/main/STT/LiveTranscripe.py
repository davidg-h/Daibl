...
class LiveTranscription:
    ...
    async def transcripe(self, audio_model, channel):
        ...
        temp_file = tempfile.NamedTemporaryFile(
            mode="w+b", dir=self.tmp_dir, delete=False
        )
        temp_file.close()
        ...
        stop_listening = self.recorder.listen_in_background(
            source, self.record_callback, phrase_time_limit=self.record_timeout
        )

        # Cue the user that we're ready to go.
        print("Live-Transcription started. Please say something.\n")
        await channel.send(
            "Live-Transcription started. Please say something.", delete_after=4
        )
